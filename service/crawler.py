...
class Crawler:
    ...
    def crawler(self):
        while True:
            ...
            # time.time()是当前的时间
            # Python中的time库时间单位是秒，其他项目都是毫秒，所以要乘以1000作为标准化
            # 制取整数部分
            ...
            try:
                ...
                # ValueError: check_hostname requires server_hostname 关掉VPN既可
                r.encoding = 'utf-8'
                self.session.close()
            ...
            # with open("onepage.html", mode="w", encoding='utf-8') as f:
            #     f.write(r.text)
            # sys.exit()
    ...
    def overall_parser(self, overall_information):
        ...
        # 因为有累计值
        # confirmedCount': 601875488, 全世界的累计病例数，所以，不需要日期去重
